...
# Secure ngrok setup
os.environ["NGROK_AUTHTOKEN"] = os.getenv("NGROK_AUTHTOKEN", "your-ngrok-token")
...
def fetch_pubmed_details(pubmed_ids):
    ...
    with open("biomed_results.xml", "w") as f:
        f.write(records)  # Removed `.decode("utf-8")`
...
def fetch_arxiv_articles(search_query, max_results=3):
    ...
    if response.status_code == 200:
        ...
        articles = [{"title": entry.title.text, "abstract": entry.summary.text, "link": entry.id.text} for entry in soup.find_all("entry")]
        return articles
    return []
